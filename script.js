...
const App = {
    ...
    async fetchBooks() {
        try {
            ...
            if (!res.ok) throw new Error('Live Server error or file not found.');
            const data = await res.json();

            this.state.books = data;
            this.state.filteredBooks = [...data];

            if (this.DOM.totalCount) this.DOM.totalCount.textContent = data.length;
            if (this.DOM.showingCount) this.DOM.showingCount.textContent = data.length;
            ...
            if (err.name === 'SyntaxError' || errorMsg.includes('JSON')) {
                ...
                errorMsg = "দয়া করে HTML ফাইলটি সরাসরি ডাবল ক্লিক করে না খুলে VS Code এর 'Live Server' দিয়ে ওপেন করুন।";
            }
    ...
    populateFilters(data) {
        if (!this.DOM.authorFilter || !this.DOM.genreFilter) return;
    ...
    filterBooks() {
        const author = this.DOM.authorFilter ? this.DOM.authorFilter.value : 'all';
        const genre  = this.DOM.genreFilter ? this.DOM.genreFilter.value : 'all';
        ...
        if (this.DOM.showingCount) this.DOM.showingCount.textContent = filtered.length;
        this.displayBooks();
    },

    displayBooks() {
        if (!this.DOM.grid) return;
        const { filteredBooks, currentPage, booksPerPage, favorites, viewMode } = this.state;
        this.DOM.grid.innerHTML = '';

        if (filteredBooks.length === 0) {
            this.DOM.grid.innerHTML = `<p style="text-align: center; margin-top: 40px; font-size: 1.1rem; width: 100%;">No books found matching your criteria.</p>`;
            this.DOM.grid.className = '';
        ...
        // TABLE VIEW
            ...
            visible.forEach((book, idx) => {
                ...
                tr.innerHTML = `
                    <td><b>#${book.collectionNumber || (start + idx + 1)}</b></td>
                    <td><img src="${this.esc(book.image)}" class="list-cover-img" alt="cover" loading="lazy"></td>
                    <td class="list-title">${this.esc(book.title)}</td>
                    <td>${this.esc(book.author)}</td>
                    <td>${this.esc(book.genre)}</td>
                    <td><i class="fa-${isFav ? 'solid' : 'regular'} fa-heart list-fav-btn ${isFav ? 'favorited' : ''}"></i></td>
                `;
                
                tr.querySelector('.list-fav-btn').addEventListener('click', (e) => {
                    e.stopPropagation();
                    this.toggleFavorite(book.title, e.target);
                });
                
                tr.addEventListener('click', () => this.openModal(book));
                tbody.appendChild(tr);
            });
            
            this.DOM.grid.appendChild(table);
        }
    ...
    toggleFavorite(title, btnElement) {
        ...
        if (isFav) {
            ...
            if (btnElement) {
                ...
                if(btnElement.tagName === 'I') {
                    btnElement.className = 'fa-regular fa-heart list-fav-btn';
                } else {
            ...
            if (btnElement) {
                ...
                if(btnElement.tagName === 'I') {
                    btnElement.className = 'fa-solid fa-heart list-fav-btn favorited';
                } else {
    ...
    openModal(book) {
        ...
        if (DOM.modalImage) DOM.modalImage.src = book.image;
        if (DOM.modalTitle) DOM.modalTitle.textContent = book.title;
        if (DOM.modalAuthor) DOM.modalAuthor.innerHTML = `<i class="fa-solid fa-pen-nib"></i> ${this.esc(book.author)}`;
        if (DOM.modalDate) DOM.modalDate.textContent = book.purchaseDate || '—';
        if (DOM.modalPurpose) DOM.modalPurpose.textContent = book.purpose || '—';
        if (DOM.modalGenre) DOM.modalGenre.textContent = book.genre || '—';
        if (DOM.modalBadge) DOM.modalBadge.textContent = `#${book.collectionNumber || ''}`;
        
        this.syncModalFavBtn(book.title);
        
        if (DOM.modalFirstPage) {
    ...
    closeModal(fromPopState = false) {
        if (!this.DOM.modal || !this.DOM.modal.classList.contains('open')) return;
    ...
    syncModalFavBtn(title) {
        if (!this.DOM.modalFavBtn) return;
    ...
    restoreTheme() {
        ...
        if (this.state.currentThemeIndex === -1) this.state.currentThemeIndex = 0;
    ...
    bindEvents() {
        ...
        if (DOM.authorFilter) DOM.authorFilter.addEventListener('change', () => this.filterBooks());
        if (DOM.genreFilter) DOM.genreFilter.addEventListener('change',  () => this.filterBooks());
        if (DOM.searchInput) DOM.searchInput.addEventListener('input',   () => this.filterBooks());
        if (DOM.searchClear) DOM.searchClear.addEventListener('click', () => { DOM.searchInput.value = ''; this.filterBooks(); });
        
        if (DOM.viewToggleBtn) {
            DOM.viewToggleBtn.addEventListener('click', (e) => {
                e.preventDefault();
                this.state.viewMode = this.state.viewMode === 'grid' ? 'list' : 'grid';
                DOM.viewToggleBtn.innerHTML = this.state.viewMode === 'grid' ? '<i class="fa-solid fa-list"></i> Booklist' : '<i class="fa-solid fa-border-all"></i> Grid View';
                this.displayBooks();
            });
        }

        if (DOM.themeToggle) DOM.themeToggle.addEventListener('click', () => this.cycleTheme());
        if (DOM.favToggle) {
        ...
        if (DOM.backdrop) DOM.backdrop.addEventListener('click', () => this.closeModal());
        
        document.addEventListener('keydown', e => { 
            if (e.key === 'Escape' && DOM.modal && DOM.modal.classList.contains('open')) this.closeModal(); 
        });
        
        window.addEventListener('popstate', (e) => {
            if (DOM.modal && DOM.modal.classList.contains('open')) {
